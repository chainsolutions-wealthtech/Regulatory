...
START=['https://crepmf.org/actuality-details?actualiteId=1000138','https://crepmf.org/publication/rapport']
UA='Mozilla/5.0 RegulatoryCorpusBot/1.0'
ALLOWED={'crepmf.org','amf-umoa.org','www.amf-umoa.org'}
def fetch(u):
...
def snippets(s):
    ...
    for key in ['actualiteid','actuality-details','actualite','publication/rapport','api/']:
        ...
        while True:
            ...
            if i<0:break
            hits.append(s[max(0,i-260):min(len(s),i+520)])
            pos=i+len(key)
            if len(hits)>=250:return hits
    return hits
...
def main():
    ...
    data={'schemaVersion':'AMF_UMOA_FRONTEND_ENDPOINT_DISCOVERY_V0_2','sourcePages':pages,'javascriptBundles':js,'candidateEndpoints':sorted(candidates),'relevantSnippets':allhits[:250],'boundary':{'diagnosticOnly':True,'endpointCandidateIsRegulatoryEvidence':False,'tlsVerificationDisabled':False}}
