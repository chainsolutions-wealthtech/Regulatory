#!/usr/bin/env python3
"""Materialize an official 2018 CREPMF/BRVM OPCVM approval decision as corroborating evidence.

This administrative act is NOT a substitute for the historical Instructions it cites.
Its sole purpose is to corroborate references/dates in the Instruction 66 historical
cross-check and to help locate institutional archives.
"""
...
EVIDENCE_ID = "INST066_HISTORICAL_CORROBORATION_NSIA_2018_165"
SOURCE_PAGE = "https://www.brvm.org/fr/crepmf-agrement-du-fonds-commun-de-placement-fcp-nsia-fonds-diversifie-en-qualite-dopcvm-sur-le"
SOURCE_URL = (
    "https://www.brvm.org/sites/default/files/"
    "20181121_-_decision_ndeg_pcr-da-2018-165_agrement_du_fcp_nsia_fonds_diversifie_en_qualite_dopcvm_sur_le_marche_financier_regional_0.pdf"
)
ROOT = Path(__file__).resolve().parents[1]
OUT = ROOT / "regulatory" / "review-evidence" / "INST066_HISTORICAL_CROSSCHECK" / "corroborating"
PDF_PATH = OUT / "DECISION_PCR_DA_2018_165_NSIA_FONDS_DIVERSIFIE.pdf"
TEXT_PATH = OUT / "DECISION_PCR_DA_2018_165_NSIA_FONDS_DIVERSIFIE.txt"
METADATA_PATH = OUT / "DECISION_PCR_DA_2018_165_NSIA_FONDS_DIVERSIFIE.metadata.json"
VALIDATION_PATH = OUT / "DECISION_PCR_DA_2018_165_NSIA_FONDS_DIVERSIFIE.validation.json"
OCR_TMP = OUT / ".nsia-2018-165.ocr.tmp.pdf"
MIN_TEXT = 500
...
def main() -> int:
    ...
    for key, regexes in PATTERNS.items():
        ...
        for regex in regexes:
            for match in re.finditer(regex, normalized, re.IGNORECASE):
                snippet = normalized[max(0, match.start() - 180): min(len(normalized), match.end() + 220)]
                matches.append(snippet.strip())
        references[key] = {
            "observed": bool(matches),
            "matchCount": len(matches),
            "snippets": unique(matches)[:5],
    ...
    metadata = {
        ...
        "interpretationBoundary": (
            "A citation in this later approval decision corroborates the existence/reference of a historical text only. "
    ...
    validation = {
        "validationId": "INST066_HISTORICAL_CORROBORATION_NSIA_2018_165_V1",
        "status": status,
        "checks": checks,
        "metrics": {
            "byteSize": byte_size,
