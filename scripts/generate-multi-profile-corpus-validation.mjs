...
const goldenMasterPath = path.join(repoRoot, "test", "fixtures", "MULTI_PROFILE_GOLDEN_MASTERS_V1.json");
const goldenMasters = JSON.parse(await readFile(goldenMasterPath, "utf8"));
...
for (const result of results) {
  const golden = goldenMasters.profiles?.[result.profile];
  assert.ok(golden, `Golden master missing: ${result.profile}`);
  assert.equal(result.generation_id, golden.generation_id, `Golden generation id drift: ${result.profile}`);
  assert.equal(
    result.prospectus_markdown_sha256,
    golden.prospectus_markdown_sha256,
    `Golden document hash drift: ${result.profile}`,
  );
  assert.equal(golden.requirement_count, 62);
  assert.equal(golden.ready_for_submission, false);
}
...
const validation = {
  ...
  checks: {
    ...
    goldenMastersMatchAttestedCi: true
  },
  profiles: results,
  goldenMasterSource: goldenMasters.source,
