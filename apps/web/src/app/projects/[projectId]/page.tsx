          ...
          <article className="content-section">
            <div className="section-heading">
              <div>
                ...
                <p>Le parcours reprend au premier groupe contenant une réponse obligatoire manquante.</p>
              </div>
            </div>
            <div className="next-action-card">
              <div>
                <strong>{nextGroup?.title ?? "Questionnaire renseigné"}</strong>
                <p>{nextGroup?.description ?? "Passez aux contrôles et aux revues humaines."}</p>
              </div>
              <Button
                href={
                  nextGroup
                    ? `/projects/${project.id}/questionnaire?group=${nextGroup.id}`
                    : `/projects/${project.id}/controls`
                }
                icon="arrow"
              >
                Continuer
            ...
            <dl className="detail-list">
              <div>
                <dt>Progression</dt>
                <dd>{calculateProgress(project)}%</dd>
              </div>
              <div>
                <dt>Version du projet</dt>
          ...
          <article className="content-section">
            <div className="section-heading">
              <div>
                ...
                <p>Les statuts restent explicites jusqu’à leur résolution.</p>
              </div>
            </div>
            <div className="alert-stack">
              {findings.slice(0, 3).map((finding) => (
                <ControlAlert key={finding.id} finding={finding} />
              ))}
            </div>
          </article>
        </section>
