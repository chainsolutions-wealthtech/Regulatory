  ...
  return (
    <ProjectWorkspaceTemplate project={project} active="preview">
      <div className="preview-layout">
        <aside className="preview-toolbar"><h2>Aperçu généré</h2><p>Le contenu est construit depuis le snapshot canonique et conserve le statut de pré-conformité.</p><GenerateProspectusButton projectId={project.id} /><dl><div><dt>Génération</dt><dd>{preview.generationId}</dd></div><div><dt>Sections</dt><dd>{preview.sections.length}</dd></div><div><dt>Soumission</dt><dd>Interdite</dd></div></dl></aside>
        <ProspectusDocument title={preview.title} sections={preview.sections} generationId={preview.generationId} readyForComplianceReview={preview.readyForComplianceReview} />
      </div>
