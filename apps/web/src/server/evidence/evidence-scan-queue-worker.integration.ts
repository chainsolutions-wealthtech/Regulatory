...
const result = await worker.runBatch({ maxItems: 10, leaseSeconds: 60 });
...
await assert.rejects(() => worker.runBatch({ maxItems: 0, leaseSeconds: 60 }), /EVIDENCE_SCAN_BATCH_SIZE_INVALID/);
await assert.rejects(() => worker.runBatch({ maxItems: 101, leaseSeconds: 60 }), /EVIDENCE_SCAN_BATCH_SIZE_INVALID/);
